...
internal abstract class HealthCheckTask @Inject constructor(
  ...
  @TaskAction
  fun checkHealth() {
    require(!extension.healthEndpoint.get().isBlank()) {
  ...
  abstract class HealthCheckWorkAction : WorkAction<HealthCheckWorkParameters> {
    ...
    private val logger: Logger = Logging.getLogger(
      "Blueground:micronaut-micronaut-benchmark:${HealthCheckTask::class.java.simpleName}"
    )
    ...
    override fun execute() {
      ...
      for (n in retries downTo 1) {
        var response: khttp.responses.Response? = null

        try {
          response = khttp.get(healthEndpoint, timeout = 2.0)
        } catch (e: Exception) {
          // catch and continue
        } finally {
          val statusCode = getStatusCode(response)
          if (statusCode == org.gradle.internal.impldep.org.apache.http.HttpStatus.SC_OK) {
            logger.quiet("Service started!")
            return
          } else {
            val msg = getStatusMessage(statusCode)
            logger.quiet("$msg. Retry in $intervalSecs sec. Remaining retries: $n")
          }
        }
    ...
    private fun getStatusCode(response: khttp.responses.Response?): Int = response?.statusCode ?: 0

    private fun getStatusMessage(statusCode: Int): String {
      return when (statusCode) {
        0 -> "Service is not responding"
        else -> "Service responded with [$statusCode]"
      }
    }
  }
}
